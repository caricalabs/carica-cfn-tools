...
@click.option('--sam-to-cfn/--no-sam-to-cfn', help='Convert the main and base template from SAM '
                                                   'to CloudFormation before applying base '
                                                   'transforms')
...
def main(stack_config_file, base_template=None, sam_to_cfn=False, extra=None):
    try:
        stack = Stack(stack_config_file, base_template=base_template, convert_sam_to_cfn=sam_to_cfn,
                      extras=extra)
        stack.create_change_set(change_set_type='CREATE')
